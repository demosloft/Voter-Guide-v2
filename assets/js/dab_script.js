...
jQuery(document).on('change keyup', '#dab_state, #dabzip_code, #dab_address', function(){
    var zip =jQuery('#dabzip_code').val(); 	var dabfull_add =jQuery('#dab_address').val();
...
function getState(zipString) {
    ...
    if (typeof zipString !== 'string') {
        jQuery('.search_errordab p.error').text('Must pass the zipcode as a string.');
        return;
    }

    /* Ensure we have exactly 5 characters to parse */
    if (zipString.length !== 5) {
        jQuery('.search_errordab p.error').text('Must pass a 5-digit zipcode.');
        return;
    }
    ...
    if (zipcode >= 35000 && zipcode <= 36999) {
        ...
        st = 'none';
        state = 'none';
        jQuery('.search_errordab p.error').text('Not a valid Zipcode');
